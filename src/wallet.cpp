...
// 421: optional setting to unlock wallet for block minting only;
...
// 421: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // 421: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            for(;;)
            {
                ...
                // 421: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
                        ...
                        if (!GetBoolArg("-avatar")) // 421: not avatar mode
                        {
...
// 421: create coin stake transaction
...
// 421: check 'spent' consistency between wallet and txindex
// 421: fix wallet spent state according to txindex
...
// 421: disable transaction (only for coinstake)
