...
BOOST_AUTO_TEST_CASE(sign)
{
    ...
    CScript standa421ripts[4];
    standa421ripts[0] << key[0].GetPubKey() << OP_CHECKSIG;
    standa421ripts[1].SetDestination(key[1].GetPubKey().GetID());
    standa421ripts[2] << key[1].GetPubKey() << OP_CHECKSIG;
    standa421ripts[3].SetDestination(key[2].GetPubKey().GetID());
    CScript evalScripts[4];
    for (int i = 0; i < 4; i++)
    {
        keystore.AddCScript(standa421ripts[i]);
        evalScripts[i].SetDestination(standa421ripts[i].GetID());
    }
    ...
    for (int i = 0; i < 4; i++)
    {
        ...
        txFrom.vout[i+4].scriptPubKey = standa421ripts[i];
    }
